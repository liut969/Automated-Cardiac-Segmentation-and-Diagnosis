    ...
    def save_image(self, images, pre_name, save_path):
        for i in range(images.shape[0]):
            for j in range(images.shape[1]):
                if i % 2 == 0:
                    save_name = os.path.join(save_path, pre_name + str(int(i/2+1)).zfill(3) + '_01_' + str(j+1).zfill(2) + '_ED.png')
                else:
                    save_name = os.path.join(save_path, pre_name + str(int(i/2+1)).zfill(3) + '_09_' + str(j+1).zfill(2) + '_ES.png')
...
if __name__ == '__main__':
    ...
    print(os.getcwd())
    data_preprocess.save_image(test_image, 'test_image', '../data/result/test_image')
