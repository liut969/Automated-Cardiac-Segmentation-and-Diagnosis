...
def save_nii_gz_data(from_path, predict_png_result_path, num, predict_nii_gz_result_path):
    ...
    for i in range(img.shape[2]):
        result_file_name = os.path.join(predict_png_result_path, 'test_image_patient' + str(num[0]) + '_frame' + str(num[1]) + '_' + str(i+1).zfill(2) + '.png')
    ...
    if num[1] == '01':
        save_name = 'patient' + str(num[0]) + '_ED.nii.gz'
    else:
        save_name = 'patient' + str(num[0]) + '_ES.nii.gz'
...
def save_data_dir(from_path, predict_png_result_path, predict_nii_gz_result_path):
    for i in range(constant_model.test_start_num, constant_model.test_end_num):
        dir_name = 'patient' + str(i).zfill(3)
        current_path = os.path.join(from_path, dir_name)
        for f_name in [f for f in os.listdir(current_path) if f.startswith('patient')]:
            if '4d' not in f_name:
                num = re.findall('\d+', f_name)
                save_nii_gz_data(os.path.join(current_path, f_name), predict_png_result_path, num, predict_nii_gz_result_path)
